...
`docker-compose.yml` поднимает три сервиса в одной сети:
...
- `backend` стартует только после того, как `postgres` прошёл `healthcheck` (`pg_isready`);
...
Единый источник правды — файл `backend/.env` (исключён из git через `.gitignore`). Содержит:
...
#### 3.7.4. Обоснование выбранного подхода
